...
const scrapNoFrills = async (items) => {
  ...
  for (const item of items) {
    ...
    const prices = await page.evaluate(() => {
      const priceTags = document.querySelectorAll(".css-epcqr3 .css-8atqhb p");
      return Array.from(priceTags)
        ...
        .map((p) => {
          ...
          if (newlineIndex !== -1) {
            const price = priceText.substring(priceText.indexOf('$'), newlineIndex);
            return { price };
          } else {
            return null;
          }
    ...
    const products = productNames
      ...
      .filter((product) => product !== null && product.price !== "Price Not Available");
...
const scrapFreshCo = async (items) => {
  ...
  for (const item of items) {
    ...
    const products = await page.evaluate((item) => {
      ...
      return Array.from(buttons)
        ...
        .map((button) => {
          ...
          const price = product.substring(product.lastIndexOf("$")).split(" ")[0];
    ...
    if (products.length > 0) {
      ...
      lowestPricedProducts.push({ productName: item, price: "Price Not Available" });
    }
  }
