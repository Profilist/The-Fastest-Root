...
function calculateSavings(firstStore, secondStore, thirdStore, fourthStore) {
  const allStores = [firstStore, secondStore, thirdStore, fourthStore];
  const itemLength = firstStore.length;
  ...
  for (let i = 0; i < itemLength; i++) {
    ...
    allStores.forEach((store) => {
      const price = parsePrice(store[i].price);
    ...
    minTotalCost += minPrice;
  }

  const savings = maxTotalCost - minTotalCost;
  const percentageSaved = 100-((savings / maxTotalCost) * 100);
...
function cheapestRouteCalculator(
  firstStore,
  secondStore,
  thirdStore,
  fourthStore,
  ...
  if (maxNumberOfStores === 2) {
      ...
      for (let a = i + 1; a < 4; a++) {
        ...
        for (let b = 0; b < firstStore.length; b++) {
          ...
          if (price1 < price2) {
            ...
            store_info.push({
              store: StoreNames[i],
              productName: AllStores[i][b].productName,
              price: AllStores[i][b].price,
            });
          } else {
            cheapest_price += price2;
            store_info.push({
              store: StoreNames[a],
              productName: AllStores[a][b].productName,
              price: AllStores[a][b].price,
            });
          }
        }
      ...
      for (let a = i + 1; a < 3; a++) {
        for (let j = a + 1; j < 4; j++) {
          ...
          for (let b = 0; b < firstStore.length; b++) {
            ...
            store_info.push({
              store: StoreNames[[i, a, j][min_index]],
              productName: AllStores[[i, a, j][min_index]][b].productName,
              price: AllStores[[i, a, j][min_index]][b].price,
            });
    ...
    for (let b = 0; b < firstStore.length; b++) {
      ...
      store_info.push({
        store: StoreNames[min_index],
        productName: AllStores[min_index][b].productName,
        price: AllStores[min_index][b].price,
      });
    }
...
module.exports = { cheapestRouteCalculator, calculateSavings };
